...
| T10 | PARTIAL | Exact-head Vercel preview is READY; real Sanity + Shopify provider E2E remains blocked on intended credentials/store data |
...
| F10 | BLOCKED | Production promotion requires explicit approval after provider-backed storefront smoke |
...
Guarded release run `35015072291` completed GREEN against the verified dependency graph: lockfile generation PASS → clean install PASS → 23/23 regression contracts PASS → TypeScript PASS → Next.js 15 production build PASS → production dependency audit PASS → synchronized `package-lock.json` commit PASS. The generated lockfile commit is `61bd72b7f47dddf5539396c2b05d98018a6b8ecd`; the clean install/audit reported 0 vulnerabilities.

Current PR head `cc2730d3fd65aa35718aa6e600bfa51b0b8f7d55` passed permanent Quality run `35015464706`. The exact-head Vercel preview on project `shop-h2r5` is `dpl_FkFrq5ByJiXDFmCDRaMexVyqsTMT` and is READY, resolving the prior build-rate delivery blocker. The older Vercel project named `shop` remains a stale duplicate with historical ERROR deployments and is not treated as the canonical preview target.
...
Status: **PARTIAL** — code/release gates and exact-head preview are green; responsive interactive QA and provider-backed commerce smoke remain the release gates.
